...
import { RoleAdmin, RoleCreateDto, RoleUpdateDto, RoleFilterDto, RolePermissionDto } from '../../../core/models/admin.model';
...
export class QuanLyVaiTroComponent implements OnInit {
    ...
    private async taiDanhSachRoles(): Promise<void> {
        try {
            ...
            this.hienThongBao('Không thể tải danh sách vai trò', 'error');
        } finally {
    ...
    private async taiDanhSachMenus(): Promise<void> {
        try {
            ...
            console.warn('Không thể tải menu:', error);
        }
    }
    ...
    async xoaRole(role: RoleAdmin): Promise<void> {
        if (role.isSystemRole) {
            this.hienThongBao('Không thể xóa vai trò hệ thống', 'error');
            return;
        }
