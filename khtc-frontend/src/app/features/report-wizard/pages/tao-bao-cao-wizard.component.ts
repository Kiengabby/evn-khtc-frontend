...
// ============================================
// Giao diện dạng stepper 5 bước, giống CMIS:
// 1. Thông tin chung  2. Đơn vị báo cáo  3. Thiết kế cột  4. Phân quyền  5. Hoàn thành
//
// === LUỒNG ===
...
export class TaoBaoCaoWizardComponent implements OnInit, OnDestroy {
    ...
    tiepTuc(): void {
        ...
        // Khi rời bước 3: lưu dữ liệu grid vào wizard state
        if (buoc === 3) {
        ...
        // Khi vào bước 3: init grid sau khi DOM render
        if (buoc + 1 === 3) {
        ...
        switch (this.buocHienTai()) {
            ...
            case 2:
                if (d.danhSachDonVi.filter(dv => dv.daChon).length === 0) {
                    loiMoi['donVi'] = 'Chọn ít nhất 1 đơn vị báo cáo';
                }
                break;
            case 3:
                if (d.danhSachNhomCot.length === 0) {
                    loiMoi['nhomCot'] = 'Tạo ít nhất 1 nhóm cột';
                }
                break;
            case 4:
                // Phân quyền không bắt buộc
                break;
        }
    ...
    capNhatField(field: string, event: Event): void {
        ...
        if (field === 'tenBaoCao' && !this.duLieu().maBaoCao) {
            const ma = val.normalize('NFD').replace(/[\u0300-\u036f]/g, '')
                .replace(/đ/gi, 'd').replace(/[^a-zA-Z0-9]/g, '_')
    ...
    private khoiTaoGrid(el: HTMLElement): void {
        ...
        this.hotInstance = new Handsontable(el, {
            ...
            cells: (row: number, col: number) => {
                ...
                // First 2 columns (STT + Nội dung) are text
                if (col <= 1) {
                ...
                // Row 0 (Tổng cộng) gets bold styling
                if (row === 0) {
    ...
    private buildDefaultGridData(): any[][] {
        ...
        const rows: any[][] = [];
        // Hàng tổng cộng
        rows.push(['', 'Tổng cộng', null, null, '=D1/C1', null, null, '=G1/F1']);
        // Hàng đơn vị
    ...
    private buildDefaultNestedHeaders(): any[][] {
        ...
        const thang = d.thangBaoCao || new Date().getMonth() + 1;
        return [
            ['STT', 'Nội dung', { label: `Thực hiện Tháng ${thang}`, colspan: 3 }, { label: 'Lũy kế từ đầu năm', colspan: 3 }],
            ['', '', 'Công suất (MW)', 'Sản lượng (MWh)', 'Số giờ (h)', 'Công suất (MW)', 'Sản lượng (MWh)', 'Số giờ (h)'],
        ];
    }
    ...
    /** Lưu dữ liệu từ grid vào wizard state */
    private luuGridVaoState(): void {
        ...
        // Cũng cập nhật danhSachNhomCot cho bước 5 summary
    ...
    private capNhatNhomCotTuGrid(): void {
        ...
        for (const item of row0) {
            ...
            for (let c = 0; c < span; c++) {
                const colName = row1 ? (typeof row1[colIdx] === 'string' ? row1[colIdx] : '') : `Cột ${c + 1}`;
    ...
    get previewCots(): { nhom: string; cot: string; loai: string }[] {
        ...
        for (const nhom of this.duLieu().danhSachNhomCot) {
            for (const cot of nhom.danhSachCot) {
                result.push({ nhom: nhom.tenNhom, cot: cot.tenCot, loai: cot.loai === 'cong_thuc' ? `Công thức: ${cot.congThuc}` : 'Nhập liệu' });
            }
        }
        return result;
    }
    ...
    async xuatBan(): Promise<void> {
        ...
        try {
            ...
            this.hienThongBao('Có lỗi xảy ra khi tạo báo cáo', 'error');
        } finally {
