...
// Lớp ánh xạ dữ liệu giữa Handsontable grid
...
export interface GridRenderConfig {
    /** Mảng 2D dữ liệu cho Handsontable */
    data: any[][];
    ...
    fixedRowsTop: number;
    /** Cấu hình ẩn cột (cột METADATA_ROW) */
...
export class GridDataMappingService {
     ...
     *
     * Vì Handsontable dùng fixedRowsTop cho header, nên dòng dữ liệu
     * đầu tiên bắt đầu ở index = fixedRowsTop.
     *
     ...
     * Dùng khi duyệt grid từ trên xuống để lấy rowCode.
     */
    ...
    /**
     * Lọc ra danh sách cột cho phép nhập liệu:
     *   - readOnly = false
    ...
    /**
     * Lấy toàn bộ dữ liệu đã nhập từ Handsontable grid.
     *
     * Quy trình:
     *   1. Xác định danh sách cột editable (readOnly=false, không phải METADATA_ROW)
     *   2. Duyệt từ dòng fixedRowsTop đến hết
     *   3. Với mỗi dòng: lấy rowCode từ cột 0 (METADATA_ROW)
     *   4. Với mỗi cột editable: nếu cell có giá trị → tạo GridCellData
     *
     * @param hotInstance - Instance Handsontable đang hiển thị
     * @param layout      - LayoutJSON từ template
     * @returns Mảng GridCellData[] chỉ chứa các ô có dữ liệu & editable
     */
    extractGridData(hotInstance: any, layout: LayoutJSON): GridCellData[] {
        const results: GridCellData[] = [];

        if (!hotInstance || !layout) {
            console.warn('[GridDataMapping] extractGridData: hotInstance hoặc layout bị null');
            return results;
        }
        ...
        // Tìm index cột METADATA_ROW (luôn phải là cột 0 theo thiết kế)
        const metadataColIndex = this.findMetadataColIndex(columns);

        if (metadataColIndex < 0) {
            console.warn('[GridDataMapping] extractGridData: Không tìm thấy cột METADATA_ROW!');
            return results;
        }

        // Duyệt từng dòng dữ liệu (bỏ qua header rows)
        for (let row = fixedRowsTop; row < totalRows; row++) {
            // Lấy rowCode từ cột ẩn METADATA_ROW
            ...
            // Kiểm tra dòng có phải readOnly không (dòng tổng/công thức)
            const rowDef = layout.rows.find(r => r.rowCode === rowCode);
            if (rowDef?.isReadOnly) {
                continue; // Bỏ qua dòng readOnly (dòng tổng/công thức)
            }

            // Duyệt từng cột editable
            for (const { colCode, colIndex } of editableCols) {
                const cellValue = hotInstance.getDataAtCell(row, colIndex);

                // Chỉ lấy ô có dữ liệu (không null, không undefined, không chuỗi rỗng)
        ...
        console.log(
            `[GridDataMapping] extractGridData: Trích xuất ${results.length} ô dữ liệu`,
            results,
        );
    ...
    /**
     * Xây dựng mảng 2D dữ liệu cho Handsontable từ template + dbData.
     *
     * Quy trình:
     *   1. Tạo mảng 2D trống (totalRows × totalCols)
     *   2. Điền rowCode vào cột 0 (METADATA_ROW) — rất quan trọng!
     *   3. Điền static text (STT, tên chỉ tiêu) từ template.rows
     *   4. Điền công thức từ mappings (nếu có)
     *   5. Ánh xạ dbData vào đúng ô [rowIndex][colIndex]
     *
     * @param layout - LayoutJSON từ template
     * @param dbData - Dữ liệu từ database
     * @returns Mảng 2D dữ liệu sẵn sàng load vào Handsontable
     */
    buildGridData(layout: LayoutJSON, dbData: GridCellData[]): any[][] {
        ...
        // ── Bước 1: Tạo mảng 2D trống ──
        const data: any[][] = [];

        // Header rows (sẽ trống, vì nestedHeaders xử lý riêng)
        ...
        // ── Bước 2: Điền rowCode vào cột METADATA_ROW (cột 0) ──
        const metadataColIndex = this.findMetadataColIndex(columns);

        if (metadataColIndex < 0) {
            console.warn('[GridDataMapping] buildGridData: Không tìm thấy cột METADATA_ROW!');
        }

        // ── Bước 3: Điền static text từ template.rows ──
        ...
        // ── Bước 4: Điền công thức từ mappings (nếu có) ──
        if (layout.mappings) {
            for (const mapping of layout.mappings) {
                if (mapping.cellRole === 'formula' && mapping.formula) {
                    ...
                    if (ri >= 0 && ci !== undefined) {
                        ...
                        console.warn(
                            `[GridDataMapping] buildGridData: Không tìm thấy vị trí cho formula mapping`,
        ...
        // ── Bước 5: Ánh xạ dbData vào đúng ô ──
    ...
    /**
     * Xây dựng toàn bộ config cho Handsontable từ layoutJSON + dbData.
    ...
    /**
     * Chuyển headerRows từ layoutJSON thành format nestedHeaders
     * mà Handsontable hiểu được.
     *
     * Handsontable nestedHeaders format:
     *   [ ["STT", "Chỉ tiêu", { label: "Năm N-1", colspan: 2 }, ...],
     *     ["STT", "Chỉ tiêu", "Thực hiện", "Kế hoạch", ...] ]
     */
    buildNestedHeaders(layout: LayoutJSON): any[][] {
        const { headerRows, columns } = layout;

        if (!headerRows || headerRows.length === 0) {
            // Fallback: 1 dòng header đơn giản từ column titles
     ...
     * Xác định readOnly, className cho từng ô dựa trên layout.
     */
    buildCellsCallback(layout: LayoutJSON): (row: number, col: number) => any {
        ...
        return (row: number, col: number): any => {
            ...
            // ── Dòng readOnly (dòng tổng/công thức) ──
            if (rowDef.isReadOnly) {
                cell.readOnly = true;
    ...
    /**
     * Xác định cột nào cần ẩn (METADATA_ROW).
     */
    ...
    /**
     * Tìm index cột METADATA_ROW.
     * Theo thiết kế luôn là cột 0, nhưng tìm để đảm bảo an toàn.
     */
    ...
    /**
     * Tạo STT tự động dựa trên level và vị trí trong mảng rows.
     *
     * Level 0: 1, 2, 3...  (hoặc "I", "II"... cho dòng tổng)
     * Level 1: 1.1, 1.2, 2.1...
     * Level 2: 1.1.1, 1.1.2...
     *
     * Nếu dòng là isReadOnly (dòng tổng), trả "" vì
     * dòng tổng thường không có STT mà có text ở cột tên.
     */
    private generateSTT(rowDef: LayoutRowDef, _currentIdx: number, _allRows: LayoutRowDef[]): string {
        // Dòng tổng/formula thường không cần STT
        if (rowDef.isReadOnly) {
            return '';
        }

        // Nếu rowCode có dạng "CHITIEU_01" → trích số để tạo STT
        // Đây là giải pháp đơn giản; cho production nên lấy STT từ template
