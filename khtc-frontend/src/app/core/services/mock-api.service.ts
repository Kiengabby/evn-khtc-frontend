// ============================================
// Service: Mock API — Giả lập Backend cho phát triển FE
// ============================================
// Service này giả lập toàn bộ API backend để FE dev độc lập.
// Khi backend sẵn sàng, chỉ cần thay MockApiService bằng ApiService thật.
//
// === CÁCH DÙNG ===
// 1. Import MockApiService thay vì ApiService trong component/service
// 2. Gọi các method giống hệt API thật (cùng input/output)
// 3. Response trả về sau 300-800ms (simulate network delay)
//
// === CÁCH CHUYỂN SANG API THẬT ===
// Thay đổi trong providers: { provide: ApiService, useClass: RealApiService }
// Hoặc dùng environment flag: environment.useMockApi
// ============================================

import { Injectable } from '@angular/core';
import { ChiTieu, ChiTieuNode, ChiTieuTaoMoi, ChiTieuCapNhat, ChiTieuBoLoc } from '../models/chi-tieu.model';
import { DonVi, DonViTaoMoi } from '../models/don-vi.model';
import { PhienBan, PhienBanTaoMoi } from '../models/phien-ban.model';
import { FormTemplate, FormTemplateTaoMoi, ColumnDefinition } from '../models/form-template.model';
import { HoSoNop, TrangThaiHoSo, PheDuyetItem, PheDuyetDto, ThongKeDashboard, TienDoEntity } from '../models/workflow.model';
import { UserAdmin, RoleAdmin, UserCreateDto, UserUpdateDto, PasswordResetDto, RoleCreateDto, RoleUpdateDto, UserFilterDto, RoleFilterDto } from '../models/admin.model';
import { User, UserRole, MenuItem } from '../models/user.model';
import { KetQuaApi, PhanTrang } from '../models/api-response.model';

@Injectable({ providedIn: 'root' })
export class MockApiService {

    // === Cache dữ liệu mock (load từ JSON) ===
    ...
    private nextId = 100; // ID tiếp theo cho bản ghi mới
    ...
    // ============================================
    // CHỈTIÊU (ACCOUNT) — CRUD
    // ============================================

    /**
     * Lấy danh sách chỉ tiêu (có phân trang + lọc)
     *
    ...
    async layDanhSachChiTieu(boLoc: ChiTieuBoLoc = {}): Promise<KetQuaApi<ChiTieu[]>> {
        ...
        return {
            trangThai: true,
            maLoi: null,
            thongBao: `Tìm thấy ${tongSoBanGhi} chỉ tiêu`,
            duLieu: ketQua,
            tongSoBanGhi,
        };
    }
    ...
    /**
     * Lấy cây chỉ tiêu (hierarchical)
     *
     * INPUT: không có
     * OUTPUT: KetQuaApi<ChiTieuNode[]> — cây chỉ tiêu đã lồng children
     */
    async layCayChiTieu(): Promise<KetQuaApi<ChiTieuNode[]>> {
        ...
        return {
            trangThai: true,
            maLoi: null,
            thongBao: 'Lấy cây chỉ tiêu thành công',
            duLieu: cayChiTieu,
        };
    }

    /**
     * Lấy chi tiết 1 chỉ tiêu theo ID
     *
     * INPUT: id (number)
    ...
    async layChiTieuTheoId(id: number): Promise<KetQuaApi<ChiTieu | null>> {
        ...
        if (!chiTieu) {
            return {
                trangThai: false,
                maLoi: 'NOT_FOUND',
                thongBao: `Không tìm thấy chỉ tiêu có ID = ${id}`,
                duLieu: null,
            };
        }
     ...
     * OUTPUT: KetQuaApi<ChiTieu> — chỉ tiêu vừa tạo (có id, ngayTao)
     */
    async taoChiTieu(dto: ChiTieuTaoMoi): Promise<KetQuaApi<ChiTieu>> {
        ...
        if (daTonTai) {
            return {
                ...
                thongBao: `Mã chỉ tiêu "${dto.maChiTieu}" đã tồn tại`,
                duLieu: null as any,
            };
        }
        ...
        return {
            trangThai: true,
            maLoi: null,
            thongBao: `Tạo chỉ tiêu "${dto.tenChiTieu}" thành công`,
            duLieu: chiTieuMoi,
        };
    }
     ...
     * OUTPUT: KetQuaApi<ChiTieu> — chỉ tiêu sau khi cập nhật
     */
    async capNhatChiTieu(dto: ChiTieuCapNhat): Promise<KetQuaApi<ChiTieu>> {
        ...
        if (index === -1) {
            return {
                trangThai: false,
                maLoi: 'NOT_FOUND',
                thongBao: `Không tìm thấy chỉ tiêu ID = ${dto.id}`,
                duLieu: null as any,
            };
        }
        ...
        return {
            trangThai: true,
            maLoi: null,
            thongBao: 'Cập nhật chỉ tiêu thành công',
    ...
    async xoaChiTieu(id: number): Promise<KetQuaApi<null>> {
        ...
        if (coCon) {
            return {
                trangThai: false,
                maLoi: 'HAS_CHILDREN',
                thongBao: `Không thể xóa "${chiTieu.tenChiTieu}" vì còn chỉ tiêu con`,
                duLieu: null,
            };
        }
        ...
        return {
            trangThai: true,
            maLoi: null,
            thongBao: `Đã xóa chỉ tiêu "${chiTieu.tenChiTieu}"`,
            duLieu: null,
        };
    }
    ...
    /**
     * Khởi tạo dữ liệu mock từ JSON files
     */
    private async khoiTaoDuLieu(): Promise<void> {
        try {
            ...
            // Tạo mock hồ sơ workflow từ dữ liệu đơn vị + biểu mẫu
            this.khoiTaoHoSo();
            
            // Tạo mock users và roles cho system admin
            this.khoiTaoAdmin();
        } catch (err) {
            console.warn('[MockApi] Không load được mock data, dùng mảng rỗng');
        }
    }

    /** Tạo dữ liệu mock hồ sơ workflow */
    private khoiTaoHoSo(): void {
        ...
        const nguoiTaos = ['Nguyễn Văn A', 'Trần Thị B', 'Lê Văn C', 'Phạm Thị D', 'Hoàng Văn E'];
        const nguoiDuyets = ['Giám đốc Toàn', 'PGĐ Kế hoạch Hương', 'Trưởng phòng KHTC Minh'];
    ...
    /** Tạo dữ liệu mock users và roles */
    private khoiTaoAdmin(): void {
        // Khởi tạo menu system
        this.danhSachMenus = [
            { menuId: 1, menuName: 'Dashboard', parentId: null, url: '/dashboard', formId: null, icon: 'pi-home', sortOrder: 1 },
            { menuId: 2, menuName: 'Dữ liệu đầu vào', parentId: null, url: '/data-entry', formId: null, icon: 'pi-table', sortOrder: 2 },
        ...
        // Khởi tạo roles với permissions
        this.danhSachRoles = [
            {
                roleId: 1, roleName: 'Super Admin', description: 'Toàn quyền hệ thống',
                permissions: [
            ...
            {
                roleId: 2, roleName: 'Giám đốc', description: 'Phê duyệt báo cáo, xem thống kê',
                permissions: [
            ...
            {
                roleId: 3, roleName: 'Kế toán trưởng', description: 'Quản lý dữ liệu kế toán, báo cáo',
                permissions: [
            ...
            {
                roleId: 4, roleName: 'Nhân viên', description: 'Nhập liệu, xem báo cáo cơ bản',
                permissions: [
        ...
        this.danhSachUsers = [
            ...
            {
                userId: 2, username: 'giamdoc', fullName: 'Nguyễn Văn Toàn', email: 'toan.gd@evn.com', phoneNumber: '024-3826-1234',
                entityCode: 'EVN', isActive: true, isLocked: false, failedLoginCount: 0,
                roles: [{ roleId: 2, roleName: 'Giám đốc' }],
                createdDate: new Date('2026-01-15'), createdBy: 'admin',
                lastLoginDate: new Date('2026-03-10'),
                notes: 'Giám đốc EVN'
            },
            {
                userId: 3, username: 'pgd.kehoach', fullName: 'Trần Thị Hương', email: 'huong.pgd@evn.com', phoneNumber: '024-3826-1235',
                entityCode: 'EVN', isActive: true, isLocked: false, failedLoginCount: 0,
                roles: [{ roleId: 2, roleName: 'Giám đốc' }],
                createdDate: new Date('2026-01-16'), createdBy: 'admin',
                lastLoginDate: new Date('2026-03-09'),
                notes: 'Phó Giám đốc phụ trách Kế hoạch'
            },
            {
                userId: 4, username: 'truong.khtc', fullName: 'Lê Văn Minh', email: 'minh.tp@evn.com', phoneNumber: '024-3826-1236',
                entityCode: 'EVN_D01', isActive: true, isLocked: false, failedLoginCount: 0,
                roles: [{ roleId: 3, roleName: 'Kế toán trưởng' }],
                createdDate: new Date('2026-01-20'), createdBy: 'admin',
                lastLoginDate: new Date('2026-03-11'),
                notes: 'Trưởng phòng KHTC'
            },
            {
                userId: 5, username: 'nv.ketoan1', fullName: 'Phạm Thị Lan', email: 'lan.pham@evn.com', 
                entityCode: 'EVN_D01', isActive: true, isLocked: false, failedLoginCount: 0,
            ...
            {
                userId: 6, username: 'nv.ketoan2', fullName: 'Hoàng Văn Nam', email: 'nam.hoang@evn.com',
            ...
            },
            {
                ...
                lastLoginDate: null,
                notes: 'Tài khoản test - đã khóa do nhập sai mật khẩu'
            }
        ];
    }
    ...
    async layDanhSachDonVi(boLoc: { tuKhoa?: string; capDonVi?: string } = {}): Promise<KetQuaApi<DonVi[]>> {
        ...
        return { trangThai: true, maLoi: null, thongBao: `Tìm thấy ${kq.length} đơn vị`, duLieu: kq };
    }

    async taoDonVi(dto: DonViTaoMoi): Promise<KetQuaApi<DonVi>> {
        await this.giaLapDelay();
        if (this.danhSachDonVi.some(d => d.maDonVi === dto.maDonVi)) {
            return { trangThai: false, maLoi: 'DUPLICATE_CODE', thongBao: `Mã "${dto.maDonVi}" đã tồn tại`, duLieu: null as any };
        }
        ...
        return { trangThai: true, maLoi: null, thongBao: `Tạo đơn vị "${dto.tenDonVi}" thành công`, duLieu: donViMoi };
    }
    ...
    async capNhatDonVi(id: number, dto: Partial<DonViTaoMoi>): Promise<KetQuaApi<DonVi>> {
        ...
        if (idx === -1) {
            return { trangThai: false, maLoi: 'NOT_FOUND', thongBao: 'Không tìm thấy đơn vị', duLieu: null as any };
        }
        this.danhSachDonVi[idx] = { ...this.danhSachDonVi[idx], ...dto, ngayCapNhat: new Date().toISOString() };
        return { trangThai: true, maLoi: null, thongBao: 'Cập nhật đơn vị thành công', duLieu: { ...this.danhSachDonVi[idx] } };
    }
    ...
    async xoaDonVi(id: number): Promise<KetQuaApi<null>> {
        ...
        if (coCon) {
            return { trangThai: false, maLoi: 'HAS_CHILDREN', thongBao: `Không thể xóa "${dv.tenVietTat}" vì còn đơn vị con`, duLieu: null };
        }
    ...
    async taoPhienBan(dto: PhienBanTaoMoi): Promise<KetQuaApi<PhienBan>> {
        ...
        if (this.danhSachPhienBan.some(d => d.maPhienBan === dto.maPhienBan)) {
            return { trangThai: false, maLoi: 'DUPLICATE_CODE', thongBao: `Mã "${dto.maPhienBan}" đã tồn tại`, duLieu: null as any };
        }
    ...
    async khoaMoPhienBan(id: number): Promise<KetQuaApi<PhienBan>> {
        ...
        const label = this.danhSachPhienBan[idx].trangThai ? 'Đã mở khóa' : 'Đã khóa';
    ...
    async layDanhSachBieuMau(boLoc: { tuKhoa?: string } = {}): Promise<KetQuaApi<FormTemplate[]>> {
        ...
        return { trangThai: true, maLoi: null, thongBao: `Tìm thấy ${kq.length} biểu mẫu`, duLieu: kq };
    }
    ...
    async layBieuMauTheoId(formId: string): Promise<KetQuaApi<FormTemplate | null>> {
        ...
        if (!bm) {
            return { trangThai: false, maLoi: 'NOT_FOUND', thongBao: `Không tìm thấy biểu mẫu "${formId}"`, duLieu: null };
        }
    ...
    async taoBieuMau(dto: FormTemplateTaoMoi): Promise<KetQuaApi<FormTemplate>> {
        ...
        if (this.danhSachBieuMau.some(b => b.formId === dto.formId)) {
            return { trangThai: false, maLoi: 'DUPLICATE_CODE', thongBao: `Mã "${dto.formId}" đã tồn tại`, duLieu: null as any };
        }
        ...
        return { trangThai: true, maLoi: null, thongBao: `Tạo biểu mẫu "${dto.formName}" thành công`, duLieu: bieuMauMoi };
    }
    ...
    async capNhatBieuMau(formId: string, dto: Partial<FormTemplateTaoMoi>): Promise<KetQuaApi<FormTemplate>> {
        ...
        if (idx === -1) {
            return { trangThai: false, maLoi: 'NOT_FOUND', thongBao: 'Không tìm thấy biểu mẫu', duLieu: null as any };
        }
        this.danhSachBieuMau[idx] = { ...this.danhSachBieuMau[idx], ...dto, ngayCapNhat: new Date().toISOString() };
        return { trangThai: true, maLoi: null, thongBao: 'Cập nhật biểu mẫu thành công', duLieu: { ...this.danhSachBieuMau[idx] } };
    }
    ...
    async layCauHinhCot(formId: string): Promise<KetQuaApi<ColumnDefinition[]>> {
        ...
        return { trangThai: true, maLoi: null, thongBao: `${cols.length} cột`, duLieu: cols };
    }
    ...
    async layDanhSachHoSo(boLoc: { tuKhoa?: string; trangThai?: TrangThaiHoSo; maDonVi?: string } = {}): Promise<KetQuaApi<HoSoNop[]>> {
        ...
        return { trangThai: true, maLoi: null, thongBao: `${kq.length} hồ sơ`, duLieu: kq };
    }
    ...
    async layHopThuPheDuyet(): Promise<KetQuaApi<PheDuyetItem[]>> {
        ...
        return { trangThai: true, maLoi: null, thongBao: `${items.length} hồ sơ chờ duyệt`, duLieu: items };
    }
    ...
    async xuLyPheDuyet(dto: PheDuyetDto): Promise<KetQuaApi<HoSoNop>> {
        ...
        if (idx === -1) {
            return { trangThai: false, maLoi: 'NOT_FOUND', thongBao: 'Không tìm thấy hồ sơ', duLieu: null as any };
        }
        ...
        this.danhSachHoSo[idx].nguoiDuyet = 'Giám đốc Toàn';
        this.danhSachHoSo[idx].ngayDuyet = new Date().toISOString();
        if (dto.ghiChu) this.danhSachHoSo[idx].ghiChu = dto.ghiChu;

        const labels: Record<string, string> = { duyet: 'Đã duyệt', tu_choi: 'Đã từ chối', tra_lai: 'Đã trả lại' };
        return {
            trangThai: true, maLoi: null,
            thongBao: `${labels[dto.hanhDong]} hồ sơ "${this.danhSachHoSo[idx].maHoSo}"`,
    ...
    async nopHoSo(hoSoId: number): Promise<KetQuaApi<HoSoNop>> {
        ...
        return { trangThai: true, maLoi: null, thongBao: `Đã nộp "${this.danhSachHoSo[idx].maHoSo}"`, duLieu: { ...this.danhSachHoSo[idx] } };
    }
    ...
    async taoUser(dto: UserCreateDto): Promise<KetQuaApi<UserAdmin>> {
        ...
        if (this.danhSachUsers.find(u => u.username === dto.username)) {
            return { trangThai: false, maLoi: 'DUPLICATE', thongBao: 'Username đã tồn tại', duLieu: null as any };
        }
    ...
    async toggleLockUser(userId: number): Promise<KetQuaApi<UserAdmin>> {
        ...
        const action = this.danhSachUsers[idx].isLocked ? 'khóa' : 'mở khóa';
        return { 
            trangThai: true, 
            maLoi: null, 
    ...
    async taoRole(dto: RoleCreateDto): Promise<KetQuaApi<RoleAdmin>> {
        ...
        // Kiểm tra role name trùng
        if (this.danhSachRoles.find(r => r.roleName === dto.roleName)) {
            return { trangThai: false, maLoi: 'DUPLICATE', thongBao: 'Tên vai trò đã tồn tại', duLieu: null as any };
        }
    ...
    async capNhatRole(dto: RoleUpdateDto): Promise<KetQuaApi<RoleAdmin>> {
        ...
        if (role.isSystemRole) {
            return { trangThai: false, maLoi: 'SYSTEM_ROLE', thongBao: 'Không thể sửa vai trò hệ thống', duLieu: null as any };
        }
    ...
    async xoaRole(roleId: number): Promise<KetQuaApi<null>> {
        ...
        const role = this.danhSachRoles[idx];
        if (role.isSystemRole) {
            return { trangThai: false, maLoi: 'SYSTEM_ROLE', thongBao: 'Không thể xóa vai trò hệ thống', duLieu: null };
        }

        if (role.userCount > 0) {
            return { trangThai: false, maLoi: 'ROLE_IN_USE', thongBao: `Vai trò đang được sử dụng bởi ${role.userCount} người dùng`, duLieu: null };
        }
